...
const PieChartComponent: React.FC<PieChartComponentProps> = ({ config, data, onCategoryClick, isPanelItem = false, onSeeData, onExportToDashboard }) => {
    ...
    const handleExportCsv = () => {
        exportToCsv(processedData, config.id || 'piechart-export');
    };

    const handleExportPng = () => {
        if (chartContainerRef.current) {
            exportToPng(chartContainerRef.current, config.id || 'piechart-export');
        }
    };
    
    const gridClasses = `col-span-12 md:col-span-${config.gridWidth || 6} row-span-${config.gridHeight || 3}`;
    const hasLegend = config.showLegend && processedData.length > 0;
