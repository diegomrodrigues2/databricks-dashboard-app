...
import type { SemicircleDonutChartWidgetConfig } from '../../types';
import WidgetExportDropdown from '../WidgetExportDropdown';
import { exportToCsv, exportToPng } from '../../utils/export';

interface SemicircleDonutChartComponentProps {
  config: SemicircleDonutChartWidgetConfig;
  data: any[];
...
const SemicircleDonutChartComponent: React.FC<SemicircleDonutChartComponentProps> = ({ config, data, onCategoryClick, isPanelItem = false, onSeeData }) => {
    ...
    const handleExportCsv = () => {
        exportToCsv(processedData, config.id || 'semicircle-donutchart-export');
    };

    const handleExportPng = () => {
        if (chartContainerRef.current) {
            exportToPng(chartContainerRef.current, config.id || 'semicircle-donutchart-export');
        }
    };
    ...
    const width = 400;
    const height = 200; // Adjusted for semicircle
                ...
                {processedData.length > 0 ? (
                    <>
                        <div className="flex-1 h-full relative">
                            <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height + 20}`} aria-label={config.title} preserveAspectRatio="xMidYMid meet">
                                <g transform={`translate(${width / 2}, ${height})`}>
                                    {pieData.map(d => {
                                        ...
                                        const rotation = (midAngleRad * 180 / Math.PI); // Adjusted rotation for semicircle
                                ...
                                {/* Center Text */}
                                {!isPanelItem && (
                                    <g transform={`translate(${width / 2}, ${height})`} className="pointer-events-none">
                                        <text textAnchor="middle" dy="-2.5em" className="fill-gray-300 text-3xl font-bold">
                        ...
                        {hasLegend && (
                             <div className="w-48 shrink-0 self-center max-h-[180px] overflow-y-auto pr-2">
...
export default SemicircleDonutChartComponent;